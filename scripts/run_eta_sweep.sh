#!/usr/bin/env bash
# eta sweep on FG10 (ps_temperature=175 fixed, var_aligned_kappa).
set -euo pipefail
cd "$(dirname "$0")/.."
...
META="scripts/logs/eta_sweep_${ts}.txt"
echo "eta sweep ts=${ts} baseline=0.75 ps_temperature=175" | tee "${META}"

launch_one() {
  local gpu="$1"
  local eta="$2"
  local tag="${eta//./p}"
  local log="scripts/logs/sweep_eta_e${tag}_gpu${gpu}_${ts}.log"
...
echo "done -> ${META}"
echo "  bash scripts/parse_hp_sweep_results.sh eta ${ts}"
