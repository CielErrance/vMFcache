#!/usr/bin/env bash
# Parallel full FG10 eval (CLIP entropy, no t-SNE).
set -euo pipefail
cd "$(dirname "$0")/.."
...
META="scripts/logs/full10_parallel_clip_entropy_${ts}.txt"
: > "${META}"
...
run_one() {
  local gpu="$1"
  local ds="$2"
  local log="scripts/logs/parallel_clip_entropy_${ds}_gpu${gpu}_${ts}.log"
...
n=0
for ds in "${DATASETS[@]}"; do
  log=$(ls scripts/logs/parallel_clip_entropy_${ds}_gpu*_${ts}.log 2>/dev/null | head -1)
  acc="NA"
  if [[ -f "${log}" ]]; then
