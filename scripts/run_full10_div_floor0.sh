#!/usr/bin/env bash
# FG10 eval with div_floor=0 (optimal hyperparams otherwise).
set -euo pipefail
cd "$(dirname "$0")/.."
...
META="scripts/logs/full10_div_floor0_${ts}.txt"
: > "${META}"
...
echo "FG10 div_floor=0 ts=${ts} ps_temperature=40 eta=0.75 rho=2 lambda_div=1" | tee -a "${META}"

run_one() {
  local gpu="$1"
  local ds="$2"
  local log="scripts/logs/div_floor0_${ds}_gpu${gpu}_${ts}.log"
...
n=0
for ds in "${DATASETS[@]}"; do
  log=$(ls scripts/logs/div_floor0_${ds}_gpu*_${ts}.log 2>/dev/null | head -1)
  acc="NA"
  if [[ -f "${log}" ]]; then
